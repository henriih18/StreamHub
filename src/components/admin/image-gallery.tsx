...
// ⚠️ IMPORTANT: No modificar funciones de carga ni eliminación sin validar con el backend.
// TODO: Optimizar preview y agregar soporte para imágenes locales.
...
type ImageType = 'streaming-type' | 'upload' | 'gallery'
...
export default function ImageGallery({ onSelectImage, currentImage }: ImageGalleryProps) {
  ...
  const loadSavedImages = async () => {
    try {
      ...
      console.log('Imágenes cargadas desde servidor:', data.images)
    } catch (error) {
  ...
  // TODO: Implementar guardado local si se permiten imágenes sin servidor
  const saveImage = (url: string, name: string) => {
    console.log('Imagen seleccionada:', name)
  }
  ...
  const deleteImage = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || 'Error al eliminar la imagen')
      }
    } catch (error) {
      toast.error('Error al eliminar la imagen')
      console.error('Delete error:', error)
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {(image.type === 'gallery' || image.type === 'upload') && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => deleteImage(image.id)}
                                ...
                                title={
                                  image.type === 'upload'
                                    ? 'Esta imagen está en el directorio principal de uploads'
                                    : undefined
                                }
                              >
                                <Trash2 className="w-4 h-4" />
