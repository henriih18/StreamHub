...
import { DollarSign, TrendingUp, TrendingDown, Calculator, RefreshCw, Calendar, History } from 'lucide-react'
...
export function ProfitsCard({ className = '' }: ProfitsCardProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        months.push({
          ...
          label: `${getMonthName(now.getMonth() + 1)} ${now.getFullYear()} (Actual)`,
        ...
        data.history?.forEach((record: any) => {
          const monthValue = `${record.year}-${record.month.toString().padStart(2, '0')}`
  ...
  const isCurrentMonth = () => {
    ...
    const now = new Date()
    return profitData.year === now.getFullYear() && profitData.month === now.getMonth() + 1
  }
      ...
      <Card className={`bg-gray-800 border-gray-700 hover:bg-gray-750 transition-colors ${className}`}>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`p-1 rounded ${isProfitable ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
              {isProfitable ? (
        ...
        <CardContent onClick={() => isCurrentMonth() && setIsExpenseManagerOpen(true)}>
          {/* Month Selector */}
          <div className="mb-3">
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-xs text-gray-500">
                  {profitData.profitMargin >= 0 ? '+' : ''}{profitData.profitMargin.toFixed(1)}% margen
                </p>
                {isCurrentMonth() && (
              ...
              <div className="mt-2 pt-2 border-t border-gray-700">
                <div className="flex justify-between text-xs">
                  <span className="text-green-400">Ingresos: {formatCurrency(profitData.revenue)}</span>
                  <span className="text-red-400">Gastos: {formatCurrency(profitData.expenses)}</span>
                </div>
            ...
            <div className="text-2xl font-bold text-gray-400">No disponible</div>
          )}
        </CardContent>
      </Card>
