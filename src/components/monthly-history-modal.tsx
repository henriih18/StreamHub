...
export function MonthlyHistoryModal({ isOpen, onClose, onSelectMonth }: MonthlyHistoryModalProps) {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [
      ['Año', 'Mes', 'Ingresos', 'Gastos', 'Ganancias', 'Margen %', 'Recargas', 'Usuarios Únicos'],
    ...
    a.href = url
    a.download = `historial-ganancias-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
      ...
      <DialogContent className="bg-gray-800 border-gray-700 text-white max-w-6xl max-h-[90vh]">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Button onClick={exportToCSV} variant="outline" size="sm" className="bg-gray-700 border-gray-600">
          ...
          <ScrollArea className="h-96">
            <div className="space-y-4">
              {historyData.yearlySummary.map((yearData: any) => (
                <Card key={yearData.year} className="bg-gray-700 border-gray-600">
                  <CardHeader 
                    className="pb-3 cursor-pointer hover:bg-gray-600/50 transition-colors"
                    onClick={() => toggleYearExpansion(yearData.year.toString())}
                  >
                    <div className="flex items-center justify-between">
                            ...
                            <CardContent className="p-3">
                                ...
                                <div className="flex items-center gap-3">
                                  <div>
                                    ...
                                    <p className="text-xs text-gray-400">
                                      {record.totalRecharges} recargas • {record.uniqueUsers} usuarios
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                  <div className="text-right">
                                    <p className={`font-bold ${
                                      record.profits >= 0 ? 'text-green-400' : 'text-red-400'
                                    }`}>
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        onSelectMonth(record.year, record.month)
                                        onClose()
                                      }}
                              ...
                              {expandedMonths.has(record.id) && (
                                <div className="mt-3 pt-3 border-t border-gray-500 grid grid-cols-2 gap-4">
                                  <div>
                                    <p className="text-xs text-gray-400 mb-1">Desglose</p>
                                    <div className="space-y-1">
                                      <div className="flex justify-between text-xs">
                                        <span>Ingresos:</span>
                                        <span className="text-green-400">{formatCurrency(record.revenue)}</span>
                                      </div>
                                      <div className="flex justify-between text-xs">
                                        <span>Gastos:</span>
                                        <span className="text-red-400">{formatCurrency(record.expenses)}</span>
                                      </div>
                                      <div className="flex justify-between text-xs">
                                        <span>Promedio recarga:</span>
                                        <span className="text-yellow-400">{formatCurrency(record.averageRecharge)}</span>
                                      </div>
                                    </div>
                                  </div>
                                  <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                      <Badge variant={record.isClosed ? "default" : "secondary"} className={
                                        record.isClosed ? "bg-green-600" : "bg-yellow-600"
                                      }>
                                        {record.isClosed ? "Cerrado" : "Activo"}
                                      </Badge>
