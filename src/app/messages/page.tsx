...
import { MessageSquare, Bell, X, Check, Trash2, AlertTriangle, Shield, Ban, Unlock, Zap, Home, ShoppingCart } from 'lucide-react'
  ...
  const markAsRead = async (messageId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('❌ Failed to mark as read:', response.status, errorMessage)
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          <div className="flex flex-col items-center space-y-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <span className="text-slate-300 text-lg font-medium">Cargando mensajes</span>
              <div className="flex space-x-1">
                <div className="w-2.5 h-2.5 bg-indigo-400 rounded-full animate-bounce" style={{ animationDelay: '450ms' }}></div>
                <div className="w-2.5 h-2.5 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '600ms' }}></div>
                <div className="w-2.5 h-2.5 bg-blue-400 rounded-full animate-bounce" style={{ animationDelay: '750ms' }}></div>
              </div>
            </div>
            <p className="text-slate-500 text-sm">Recibiendo tus notificaciones...</p>
          </div>

          {/* Message progress bar */}
          <div className="w-80 h-2 bg-slate-700/50 rounded-full overflow-hidden backdrop-blur-sm">
            <div className="h-full bg-gradient-to-r from-blue-500 via-cyan-500 to-indigo-500 rounded-full animate-pulse-slow relative overflow-hidden"
                 style={{ 
                   width: '70%',
          ...
          <div className="flex space-x-4">
            <div className="w-8 h-8 bg-blue-500/20 rounded-lg flex items-center justify-center animate-float" style={{ animationDelay: '0ms' }}>
              <MessageSquare className="w-4 h-4 text-blue-400" />
            </div>
            <div className="w-8 h-8 bg-cyan-500/20 rounded-lg flex items-center justify-center animate-float" style={{ animationDelay: '500ms' }}>
              <Bell className="w-4 h-4 text-cyan-400" />
            </div>
            <div className="w-8 h-8 bg-indigo-500/20 rounded-lg flex items-center justify-center animate-float" style={{ animationDelay: '1000ms' }}>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400">
              {unreadCount > 0 ? `${unreadCount} mensaje${unreadCount > 1 ? 's' : ''} no leído${unreadCount > 1 ? 's' : ''}` : 'No hay mensajes nuevos'}
            </p>
          </div>
          {unreadCount > 0 && (
            <Button
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-1">
                                <h3 className={`font-medium text-sm truncate ${
                                  message.isRead ? 'text-slate-400' : 'text-white'
                                }`}>
                                  {message.title}
                                </h3>
                      ...
                      <div>
                        ...
                        <CardDescription className="text-slate-400">
                          ...
                          {selectedMessage.sender.role === 'ADMIN' && (
                            <Badge className="ml-2 bg-purple-600">Administrador</Badge>
                          )}
                        </CardDescription>
                      </div>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-slate-500 text-sm">
                        {new Date(selectedMessage.createdAt).toLocaleDateString('es-CO', {
                          weekday: 'long',
                          year: 'numeric',
                          month: 'long',
                          day: 'numeric',
                    ...
                    {selectedMessage.type.includes('BLOCK') || selectedMessage.type.includes('RESTRICTION') ? (
                      <div className="bg-red-900/20 border border-red-700/50 rounded-lg p-4">
                        <p className="text-red-300 text-sm">
                          <strong>Importante:</strong> Si tienes preguntas sobre esta acción, por favor contacta al soporte.
                        </p>
                      </div>
                    ) : null}
                  </div>
              ...
              <Card className="bg-slate-800 border-slate-700">
                <CardContent className="flex items-center justify-center h-96">
                  <div className="text-center">
                    ...
                    <h3 className="text-white text-lg font-medium mb-2">Selecciona un mensaje</h3>
                    <p className="text-slate-400">
