...
import { User, ShoppingBag, CreditCard, ChevronDown, ChevronUp, Mail, Shield, AlertCircle, CheckCircle, Phone, Globe, TrendingUp, Headphones, MessageSquare, Send, Monitor, RefreshCw } from 'lucide-react'
...
export default function AccountPage() {
      ...
      if (response.ok) {
        ...
        if (userData.user) {
          ...
          if (userData.user.createdAt) {
            userData.user.memberSince = new Date(userData.user.createdAt).toLocaleDateString('es-CO', {
              day: '2-digit',
              month: '2-digit',
              year: 'numeric'
            })
  ...
  const getTotal = () => {
    return (cartItems || []).reduce((total, item) => total + (item.streamingAccount.price * item.quantity), 0)
  }
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">No has iniciado sesión</h2>
          <p className="text-slate-400 mb-6">Por favor inicia sesión para ver tu cuenta</p>
          <Button 
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-slate-400">Gestiona tu perfil y tus cuentas de streaming</p>
          </div>
              ...
              <div className="flex items-center gap-6">
                <div className="w-16 h-16 bg-emerald-600 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold text-xl">
                    {user.fullName?.charAt(0).toUpperCase() || user.username?.charAt(0).toUpperCase() || user.email.charAt(0).toUpperCase()}
                  </span>
                </div>
                <div className="flex-1">
                  ...
                  <div className="space-y-2">
                    ...
                    {user.username && (
                      <div className="flex items-center gap-2 text-slate-400">
                        <User className="w-4 h-4" />
                        <span className="text-sm">Usuario: {user.username}</span>
                      </div>
                    )}
                    {/* Email */}
                    <div className="flex items-center gap-2 text-slate-400">
                ...
                <div className="text-right">
                  ...
                  <Badge className={`${user.role === 'ADMIN' ? 'bg-purple-600' : 'bg-emerald-600'} text-white mb-2`}>
          ...
          <Card className="bg-slate-800 border-slate-700 mb-8">
            ...
            <CardContent>
              {(supportContacts || []).filter(contact => contact.isActive).length === 0 ? (
                <div className="text-center py-8">
                  <Headphones className="w-12 h-12 text-slate-600 mx-auto mb-4" />
                  <h3 className="text-lg font-semibold text-white mb-2">Soporte no disponible</h3>
                  <p className="text-slate-400">No hay contactos de soporte configurados</p>
                </div>
              ) : (
                          ...
                          <div>
                            ...
                            <Badge className="bg-emerald-600/20 text-emerald-300 text-xs">
                              {contact.type === 'whatsapp' ? 'WhatsApp' : 
                               contact.type === 'phone' ? 'Teléfono' : 
                               contact.type === 'telegram' ? 'Telegram' : 'SMS'}
                            </Badge>
                          </div>
                        </div>
                        <p className="text-emerald-400 font-medium mb-2">{contact.number}</p>
                        {contact.description && (
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl font-bold text-white">Mis Pedidos</CardTitle>
                  <CardDescription className="text-slate-400">
                ...
                <div className="text-right">
                  <div className="text-sm text-slate-400">
                    {(orders || []).filter(o => o.status === 'COMPLETED').length} activos
                  </div>
                </div>
              </div>
            </CardHeader>
            ...
            <CardContent>
              {loadingOrders ? (
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-semibold text-white mb-2">No tienes pedidos</h3>
                  <p className="text-slate-400 mb-6">Explora nuestro catálogo y adquiere tu primera cuenta</p>
                  <Button
                ...
                <div className="space-y-4">
                  {orders.map((order) => (
                    <div key={order.id} className="bg-slate-700 rounded-lg p-6 border border-slate-600">
                      {/* Header */}
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-3">
                            <h3 className="font-semibold text-white text-lg">
                              {order.streamingAccount?.name || 'Cuenta Exclusiva'}
                            </h3>
                            <Badge className={`${
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-slate-400">
                                {order.saleType === 'FULL' ? 'Cuenta Completa' : 'Perfil'} • {order.streamingAccount?.duration}
                              </p>
                            </div>
                      ...
                      {(order.accountEmail || order.accountPassword || order.profileName || order.profilePin) && (
                            ...
                            {order.accountEmail && (
                              <div className="group relative overflow-hidden rounded-lg border border-emerald-500/20 bg-slate-800/50 backdrop-blur-sm transition-all hover:border-emerald-500/40 hover:bg-slate-800/70">
                                <div className="flex items-center justify-between p-4">
                                  ...
                                  <div className="flex items-center gap-3">
                                    <span className="text-emerald-300 font-mono font-semibold">{order.accountEmail}</span>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-8 w-8 p-0 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 transition-all"
                                      onClick={() => {
                                        navigator.clipboard.writeText(order.accountEmail!)
                            ...
                            {order.accountPassword && (
                              <div className="group relative overflow-hidden rounded-lg border border-emerald-500/20 bg-slate-800/50 backdrop-blur-sm transition-all hover:border-emerald-500/40 hover:bg-slate-800/70">
                                <div className="flex items-center justify-between p-4">
                                  <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-emerald-400"></div>
                                    <span className="text-slate-300 font-medium">Contraseña</span>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    <span className="text-emerald-300 font-mono font-semibold">{order.accountPassword}</span>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-8 w-8 p-0 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 transition-all"
                                      onClick={() => {
                                        navigator.clipboard.writeText(order.accountPassword!)
                            ...
                            {order.profileName && (
                              <div className="group relative overflow-hidden rounded-lg border border-emerald-500/20 bg-slate-800/50 backdrop-blur-sm transition-all hover:border-emerald-500/40 hover:bg-slate-800/70">
                                <div className="flex items-center justify-between p-4">
                                  <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-emerald-400"></div>
                                    <span className="text-slate-300 font-medium">Perfil</span>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    <span className="text-emerald-300 font-semibold">{order.profileName}</span>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-8 w-8 p-0 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 transition-all"
                                      onClick={() => {
                                        navigator.clipboard.writeText(order.profileName!)
                                        toast.success('Nombre de perfil copiado')
                                      }}
                                    >
                                      <CreditCard className="w-4 h-4" />
                                    </Button>
                            ...
                            {order.profilePin && (
                              <div className="group relative overflow-hidden rounded-lg border border-emerald-500/20 bg-slate-800/50 backdrop-blur-sm transition-all hover:border-emerald-500/40 hover:bg-slate-800/70">
                                <div className="flex items-center justify-between p-4">
                                  <div className="flex items-center gap-3">
                                    <div className="w-2 h-2 rounded-full bg-emerald-400"></div>
                                    <span className="text-slate-300 font-medium">PIN</span>
                                  </div>
                                  <div className="flex items-center gap-3">
                                    <span className="text-emerald-300 font-mono font-semibold">{order.profilePin}</span>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                                      className="h-8 w-8 p-0 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 transition-all"
                                      onClick={() => {
                                        navigator.clipboard.writeText(order.profilePin!)
                          ...
                          <div className="flex gap-3 mt-6 pt-4 border-t border-emerald-500/20">
                            <Button
                              ...
                              onClick={() => {
                                ...
                                if (order.profilePin) credentials.push(`PIN: ${order.profilePin}`)
                                
                                navigator.clipboard.writeText(credentials.join('\n'))
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                              ...
                              <div className="space-y-3">
                                <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                  <label className="text-slate-400 text-sm">ID del Pedido</label>
                                  <div className="text-white font-mono text-sm">{order.id}</div>
                                </div>
                                
                                <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                  <label className="text-slate-400 text-sm">Tipo de Venta</label>
                                ...
                                <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                  <label className="text-slate-400 text-sm">Cantidad</label>
                                  <div className="text-white">
                                ...
                                <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                  ...
                                  <div className="text-white">
                                    ${(order.totalPrice / order.quantity).toLocaleString('es-CO')}
                                  </div>
                                </div>
                                
                                <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                  <label className="text-slate-400 text-sm">Fecha de Compra</label>
                                ...
                                {order.expiresAt && (
                                  <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                    <label className="text-slate-400 text-sm">Fecha de Expiración</label>
                                    <div className="text-white text-sm">
                            ...
                            {order.streamingAccount && (
                              <div>
                                ...
                                <div className="space-y-3">
                                  <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                    <label className="text-slate-400 text-sm">Calidad</label>
                                  ...
                                  <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                    <label className="text-slate-400 text-sm">Pantallas</label>
                                    <div className="text-white font-semibold">
                                  ...
                                  <div className="p-3 bg-slate-800 rounded border border-slate-600">
                                    <label className="text-slate-400 text-sm">Duración</label>
                                    <div className="text-white font-semibold">
