...
export async function POST() {
  try {
    ...
    //console.log(`Reservas eliminadas: ${deletedReservations.count}`)

    // Encontrar y eliminar cart items con reservas expiradas
    ...
    //  Eliminar los items del carrito expirados
    ...
    const affectedCartIds = [
      ...new Set(expiredItems.map((item) => item.cartId)),
    ];
    let updatedCarts = 0;
    ...
    // NOTIFICAR A USUARIOS AFECTADOS VIA WEBSOCKET
    try {
      ...
      if (io) {
        ...
        const affectedUsers = [
          ...new Set(
            expiredItems.map((item) => item.cart?.userId).filter(Boolean)
          ),
        ];
        ...
        affectedUsers.forEach((userId) => {
          ...
          io.to(`user:${userId}`).emit("cartExpired", {
            ...
            expiredItems: expiredItems
              .filter((item) => item.cart?.userId === userId)
              .map((item) => ({
                id: item.id,
                name:
