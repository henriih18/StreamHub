...
export async function POST(request: NextRequest) {
  try {
    ...
    */
    ...
    const parsedPricePerProfile = pricePerProfile ? parseFloat(pricePerProfile) : null

    // Validate required fields
    if (!name || !description || !type || isNaN(parsedPrice) || parsedPrice < 0 || !parsedDuration || parsedDuration <= 0) {
      ...
      return NextResponse.json(
        { error: 'Todos los campos requeridos deben ser completados con valores válidos' },
        { status: 400 }
      )
    }
