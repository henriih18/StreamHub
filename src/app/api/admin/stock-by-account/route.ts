...
export async function GET(request: NextRequest) {
  try {
    ...
    const exclusiveStockData = exclusiveAccounts.map(account => ({
      ...
      accountStock: account.saleType === 'FULL' ? account.exclusiveStocks.length : 0,
      profileStock: account.saleType === 'PROFILES' ? account.exclusiveStocks.length : 0,
    ...
    const totalAccounts = allStockData.reduce((sum, account) => sum + account.accountStock, 0)
    const totalProfiles = allStockData.reduce((sum, account) => sum + account.profileStock, 0)
    ...
    const response = {
      ...
      data: {
        ...
        byStreamingType: Object.values(stockByStreamingType).sort((a: any, b: any) => b.totalStock - a.totalStock),
