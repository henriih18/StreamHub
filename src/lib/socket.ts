...
export const broadcastMessageUpdate = (io: Server, userId: string, unreadCount: number) => {
...
async function getRealTimeStats() {
  try {
    ...
    const onlineUsersResponse = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/admin/online-users`);
    let onlineUsers = {
      current: 0,
      peakToday: 0,
      averageToday: 0
    ...
    if (onlineUsersResponse.ok) {
      ...
      onlineUsers = {
        ...
        averageToday: onlineData.averageToday || Math.floor(Math.random() * 40) + 15
      };
    }
    ...
    const totalRegularStock = regularAccounts.reduce((sum, account) => 
      sum + (account._count.accountStocks || 0) + (account._count.profileStocks || 0), 0
    );
    
    const totalExclusiveStock = exclusiveAccounts.reduce((sum, account) => 
      sum + (account.exclusiveStocks?.filter(stock => stock.isAvailable).length || 0), 0
    );
